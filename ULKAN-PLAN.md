...
- ✅ **Публичный Трекинг `/track`**: поиск заявки по номеру (без входа — /track и /api/track в PUBLIC
  middleware), прогресс-бар стадий (Принята→Приёмка→Доставка→Учёт→Завершена), статусы позиций,
  отмена. Бэкенд `track(id)`, GET /api/track?id=. Build ✅.
- ✅ **Портал филиала `/branch/[slug]`**: заявки своего филиала (вкладки Входящие/Исходящие по org)
  + Новая заявка (создание через /api/client/orders). BranchPortal.
- ✅ **Портал склада `/warehouse/[slug]`**: остатки выбранного склада (по /api/refs+/api/stock),
  итоговая стоимость. WarehousePortal (read-only). Build ✅.
- **ВСЕ 5 ПОРТАЛОВ ГОТОВЫ** (Логист/Клиент/Трекинг/Филиал/Склад).
- ✅ **Ф5 накладные (соединение) — заявка→документ**: после «Бухгалтерии» разделы **Приходные накладные**
  (закуп/возврат) и **Расходные накладные** (продажа). Берут карточки с «К учёту», кнопка «Провести» →
  ERP-документ через `invoice.service.postOrderInvoice` (createPurchase/createSale: документ+склад+FIFO),
  карточка → Бухгалтерия с linkedDocId. `refsRepo.centralWarehouse`, POST /api/orders/[id]/invoice,
  `InvoicesScreen`. Smoke ✅: закуп→приход +склад, продажа→расход −склад, повтор блокируется.
  **Теперь заявки наполняют Финансы/Склад/Рентабельность реальными документами.**
- ✅ **Филиал-магазин «Магазин Кристалл» (kind=seller) — открытие:** контрагенты перенесены из 1С
  «КРИСТАЛЛ 2025» (90 шт, орг-владелец = Кристалл, тип «Клиент+Поставщик»); кабинет `/branch/[slug]`
  ветвится по виду орг → `SellerPortal` = **касса продавца с телефона**: 📖 Каталог (`NomPicker`,
  цвет→размер→кол-во) → чек (± кол-во, правка цены, цены из `/api/pricing`) → нал/каспи/QR/сдача →
  «💵 Продать». Бэк: `POST /api/branch/sell` → `cashier.service.sellDirect` (карточка-продажа в книге
  филиала + расходная со «Склад Кристалл» + оплаты; долг = дебиторка филиала, БЕЗ цепочки на головной);
  покупатель по умолчанию «Розничный покупатель», выбор — `ContragentPicker`. Вкладки: Касса/Чеки
  (с отменой продажи)/Склад/Смена/Док-ты/Финансы. Смоук ✅ (чек нал+каспи, склад −, продажа в долг,
  сторно). ⏭ Дальше: приход товара от головного, остатки/начальные остатки Кристалла.
- ⏭ Осталось (опц.): Ф4 Автозакуп (сводка потребности→черновик→ProcurementLink); возвраты в заявках;
  чат по карточке, суточные отчёты логиста, реалтайм (Pusher+поллинг).
