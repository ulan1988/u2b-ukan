...
- ⏭ Дальше по одному экрану (Приёмка с формой создания, Исходящие, К учёту, Бухгалтерия, Архив,
  Дашборд, Закуп-отчёт, Склад, Номенклатура, Настройки) + модалка карточки (позиции/история/чат).
  Потом порталы (Логист/Филиал/Клиент/Склад/Трекинг) и «включение» ERP-страниц как разделов.
