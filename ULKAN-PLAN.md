...
- ⏭ Далее: порталы по ролям (Логист /rsp, Филиал /branch, Клиент /client, Склад /warehouse, Трекинг /track);
  затем Ф4 автозакуп → цепочка + Ф5 авто-документы (закрытие закупа→приход, продажи→расход).
