...
- ⏭ Дальше по одному экрану (Исходящие, К учёту, Бухгалтерия, Архив, Дашборд, Закуп-отчёт, Склад,
  Номенклатура, Настройки) + модалка карточки (позиции/история/чат). Потом порталы и «включение»
  ERP-страниц как разделов (СПРОСИТЬ у пользователя маппинг: Финансы→Бухгалтерия и т.д.).
