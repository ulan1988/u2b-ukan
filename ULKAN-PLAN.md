...
- ⏭ Дальше: Дашборд, Закуп-отчёт, Склад, Номенклатура, Настройки, История, Фильтр + модалка карточки
  (позиции/история/чат). Потом порталы и «включение» ERP-страниц как разделов
  (СПРОСИТЬ маппинг: Финансы→Бухгалтерия и т.д.).
