...
export async function resolveTarget(session: any, uid?: string | null) {
  ...
  if (uid && isAdmin) {
    ...
    if (u) return { id: u.id, name: u.name, orgId: u.orgId, contragentId: (u as any).contragentId ?? null }
  }
  return { id: session.id, name: session.name, orgId: session.orgId, contragentId: (session as any).contragentId ?? null }
}
