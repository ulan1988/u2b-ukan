...
export const TRANSITIONS: Record<string, Transition> = {
  ...
  // Производитель: взял заказ на стол мастера (раскрой + гибка) — статус «К выполнению».
  produceAccept: {
    roles: [...ADMIN, 'branch'],
    patch: () => ({ status: 'К выполнению' }),
    history: () => 'Взято к выполнению (раскрой/гибка)',
  },
  // Производитель: раскроил и согнул — статус «Выполнено». Дальше отдельным шагом «К логисту».
  produceDone: {
    roles: [...ADMIN, 'branch'],
