...
import { statusStyle, cardSum, cardProgress, barColor, fmtMoney, fmtDate } from '@/lib/adminFmt'
import { clientOrders, createClientOrder } from '@/lib/api/orders'
...
export default function ClientApp({ user }: { user: { name: string } }) {
  ...
  const load = useCallback(() => { clientOrders().then(setOrders) }, [])
  ...
  async function submit() {
    ...
    if (!positions.length) { setBusy(false); setMsg('Добавьте хотя бы одну позицию'); return }
    const r = await createClientOrder({ comment, positions })
    setBusy(false)
    if (!r.ok) { setMsg('Ошибка отправки'); return }
    setRows([{ productId: '', qty: '1' }]); setComment(''); setTab('orders'); load()
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: COLORS.bg, fontFamily: "'Golos Text', system-ui, sans-serif" }}>
      <div style={{ background: COLORS.dark, color: '#fff', padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div style={{ flex: 1 }}><div style={{ fontWeight: 800, fontSize: 15 }}>Кабинет заказчика</div><div style={{ fontSize: 12, color: COLORS.sidebar.muted }}>{user.name}</div></div>
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: 20 }}>
        <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
          {([['orders', 'Мои заявки'], ['new', '＋ Новая заявка']] as const).map(([k, l]) => (
        ...
        {tab === 'orders' ? (
          orders.length === 0 ? <div style={{ background: '#fff', borderRadius: 14, padding: 28, boxShadow: '0 0 0 1.5px #e6e2dc', color: COLORS.textMuted, fontSize: 14 }}>Заявок пока нет</div>
            : orders.map(o => {
              ...
              return (
                <div key={o.id} style={{ background: '#fff', borderRadius: 14, padding: 16, marginBottom: 12, boxShadow: '0 1px 4px rgba(0,0,0,.05)' }}>
                  ...
                  <div style={{ fontSize: 13, color: COLORS.textLight, marginBottom: 8 }}>{(o.positions || []).map((p: any) => `${p.name1c || p.oral} ×${Number(p.qty)}`).join(', ')}</div>
                  <div style={{ height: 6, background: '#f1efec', borderRadius: 4, overflow: 'hidden', marginBottom: 6 }}><div style={{ height: '100%', width: `${pct}%`, background: barColor(pct) }} /></div>
                  <div style={{ textAlign: 'right', fontWeight: 700, fontSize: 14 }}>{fmtMoney(cardSum(o))} ₸</div>
                </div>
              )
            })
        ) : (
          <div style={{ background: '#fff', borderRadius: 14, padding: 20, boxShadow: '0 0 0 1.5px #e6e2dc' }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: '#5f5952', marginBottom: 12 }}>ПОЗИЦИИ ЗАЯВКИ</div>
            {rows.map((r, i) => (
            ...
            <textarea style={{ ...inp, minHeight: 56, resize: 'vertical', marginBottom: 12 }} placeholder="Комментарий / пожелания" value={comment} onChange={e => setComment(e.target.value)} />
            {msg && <div style={{ color: '#b03020', fontSize: 13, marginBottom: 10 }}>{msg}</div>}
            <button disabled={busy} onClick={submit} style={{ padding: '11px 22px', background: COLORS.primary, color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit', opacity: busy ? 0.5 : 1 }}>{busy ? 'Отправка…' : 'Отправить заявку'}</button>
          </div>
        )}
      </div>
    </div>
