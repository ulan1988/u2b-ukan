...
export default function RouteModal({ docId, onClose }: { docId: string; onClose: () => void }) {
  ...
  const pos = (n: any) => ({ x: 24 + (n.level || 0) * COLW, y: 24 + ((n.lane || 0) - minLane) * ROWH })
  const W = Math.max(600, 24 + (Math.max(0, ...nodes.map(n => n.level || 0)) + 1) * COLW)
  const H = Math.max(240, 48 + (Math.max(0, ...nodes.map(n => (n.lane || 0) - minLane)) + 1) * ROWH)
              ...
              <div style={{ position: 'relative', width: W, height: H }}>
                <svg width={W} height={H} style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
                  {edges.map((e, i) => {
                    ...
                    return <path key={i} d={`M ${x1} ${y1} C ${x1 + k} ${y1}, ${x2 - k} ${y2}, ${x2} ${y2}`} fill="none" stroke={col} strokeWidth={active ? 2.2 : 1.5} strokeDasharray={ret || link ? '5 5' : undefined} style={{ transition: 'stroke .15s' }} />
                  })}
                </svg>
                {nodes.map(n => {
