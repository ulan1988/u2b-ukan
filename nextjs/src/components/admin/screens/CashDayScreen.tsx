                ...
                {(data.staff || []).length === 0 ? <div style={{ fontSize: 13, color: COLORS.textMuted, padding: '6px 0' }}>Нет сотрудников</div>
                  : (data.staff || []).map((u: any) => (
                    <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0', borderBottom: `1px solid ${COLORS.borderLight}` }}>
                      ...
                      <input value={wages[u.id] || ''} inputMode="decimal" onChange={e => setWages(w => ({ ...w, [u.id]: e.target.value.replace(/[^0-9.,]/g, '') }))} placeholder="0" style={{ width: 130, padding: '7px 9px', borderRadius: 7, border: `1.5px solid ${COLORS.border}`, fontSize: 14, fontWeight: 700, textAlign: 'right', fontFamily: 'inherit' }} />
                    </div>
                  ))}
