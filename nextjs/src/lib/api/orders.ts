...
export async function payCard(cardId: string, body: { cash?: number; kaspi?: number; qr?: number; change?: number; changeFrom?: string; localDebt?: boolean }) {
...
export async function returnSale(body: { uid?: string; cardId: string; items?: { posId: string; qty: number }[]; accountId: string }) {
  ...
  return {
    ...
    number: r.data?.number as string | undefined,
    refund: r.data?.refund as number | undefined,
    returned: r.data?.returned as number | undefined,
  }
}
