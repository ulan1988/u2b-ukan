...
import { createOrder, listByScreen } from '@/services/order.service'
...
export async function GET(req: NextRequest) {
  ...
  const screen = searchParams.get('screen') || 'incoming'
  if (!orgId) return NextResponse.json({ error: 'orgId обязателен' }, { status: 400 })
  return NextResponse.json(await listByScreen(orgId, screen))
}
